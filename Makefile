...
test-integration: vet mocks
	sudo docker-compose -f $(test_docker_compose_file) up -d
...
build-docker:
	...
	sudo docker build -f docker/hs_auth/Dockerfile -t hs_auth . ;

# sets up the hacker suite docker network
setup-network:
	@echo "=============setting up the hacker suite network============="
	sudo docker network create --driver bridge hacker_suite || echo "This is most likely fine, it just means that the network has already been created"

# starts the app and MongoDB in docker containers
up: vet build-docker setup-network
	@echo "=============starting hs_auth============="
	sudo docker-compose -f $(prod_docker_compose_file) up -d
...
up-dev: vet setup-network
	...
	sudo docker-compose -f $(dev_docker_compose_file) up -d
	refresh run

# prints the logs from all containers
logs:
ifeq ($(ENV), dev)
	sudo docker-compose -f $(dev_docker_compose_file) logs -f
else
	sudo docker-compose -f $(prod_docker_compose_file) logs -f
endif

# prints the logs only from the go app
logs-app:
	sudo docker-compose -f $(prod_docker_compose_file) logs -f hs_auth

# prints the logs only from the database
logs-db:
ifeq ($(ENV), dev)
	sudo docker-compose -f $(dev_docker_compose_file) logs -f mongo
else
	sudo docker-compose -f $(prod_docker_compose_file) logs -f mongo
endif

# shuts down the containers
down:
	sudo docker-compose -f $(prod_docker_compose_file) down
	sudo docker-compose -f $(dev_docker_compose_file) down
	sudo docker-compose -f $(test_docker_compose_file) down
