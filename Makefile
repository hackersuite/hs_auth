...
# build target for CI
ci: vet
	sudo grep -rl --exclude "./vendor/*" --include "*.go" "interface {" . | while read -r file ; do mockgen --source=$$file --destination mocks/$$file ; done | docker-compose -f $(test_docker_compose_file) up -d
