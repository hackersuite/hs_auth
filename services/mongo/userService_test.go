...
func Test_UpdateUserWithID__should_return_err_when_update_params_are_invalid(t *testing.T) {
	uService, _, cleanup := setupUserTest(t)
	defer cleanup()

	assert.Equal(t, services.ErrInvalidUserUpdateParams,
		uService.UpdateUserWithID(context.Background(), "testid", services.UserUpdateParams{
			entities.UserID: 3,
		}))
}

func Test_UpdateUsersWithAuthLevel__should_return_err_when_update_params_are_invalid(t *testing.T) {
	uService, _, cleanup := setupUserTest(t)
	defer cleanup()

	assert.Equal(t, services.ErrInvalidUserUpdateParams,
		uService.UpdateUsersWithAuthLevel(context.Background(), authlevels.Organizer, services.UserUpdateParams{
			entities.UserID: 3,
		}))
}

func Test_UpdateUsersWithTeam__should_return_err_when_update_params_are_invalid(t *testing.T) {
	uService, _, cleanup := setupUserTest(t)
	defer cleanup()

	assert.Equal(t, services.ErrInvalidUserUpdateParams,
		uService.UpdateUsersWithTeam(context.Background(), "test id", services.UserUpdateParams{
			entities.UserID: 3,
		}))
}
func Test_UpdateUserWithEmail__should_return_err_when_update_params_are_invalid(t *testing.T) {
	uService, _, cleanup := setupUserTest(t)
	defer cleanup()

	assert.Equal(t, services.ErrInvalidUserUpdateParams,
		uService.UpdateUserWithEmail(context.Background(), "test email", services.UserUpdateParams{
			entities.UserID: 3,
		}))
}